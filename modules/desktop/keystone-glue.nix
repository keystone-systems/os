    ...
    home-manager.sharedModules = [
      (
        ...
        }:
        {
          ...
          # Setting this to null would gate off every Walker menu, including
          # the main menu that Mod+Escape opens and the hardware menu that
          # runs `ks approve -- keystone-enroll-fido2`. Both still work: the
          # only casualty of the Rust CLI removal is the update *entry*,
          # whose backend (`ks menu update`) is now an inert stub.
