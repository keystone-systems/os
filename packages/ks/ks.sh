...
# Prefer the YubiKey age identity when the caller has not chosen one, so the
# two commands that decrypt — `rekey` and `edit` — cannot drift apart on which
# key they use. `sync` only rewrites .sops.yaml and never calls this.
#
# The identity file lists EVERY registered key. age tries each stub in turn, so
# handing sops the whole file makes it ask for a key that is not plugged in and
# wait for the operator to skip it. Offer only the keys ykman reports as
# connected. Both comment formats are accepted: `# serial:NNN` as written by
# the age-yubikey module, and `# Serial: NNN, Slot: N` as printed by
# `age-plugin-yubikey --list`.
SOPS_IDENTITY_TMPDIR=""
sops_identity_cleanup() {
  [ -n "$SOPS_IDENTITY_TMPDIR" ] || return 0
  rm -rf "$SOPS_IDENTITY_TMPDIR"
  SOPS_IDENTITY_TMPDIR=""
}

sops_age_identity() {
  local identity="${SOPS_AGE_KEY_FILE:-$HOME/.age/yubikey-identity.txt}"
  [ -f "$identity" ] || return 0
  export SOPS_AGE_KEY_FILE="$identity"

  # A file with no plugin stub needs no device; keep software identities and
  # do not make ykman a dependency of every secrets command.
  grep -q '^AGE-PLUGIN-YUBIKEY-' "$identity" || return 0
  command -v ykman >/dev/null || {
    note "ykman is not installed; offering every registered YubiKey identity"
    return 0
  }

  local present
  present="$(ykman list --serials 2>/dev/null | grep -E '^[0-9]+$' || true)"
  if [ -z "$present" ]; then
    note "no YubiKey is connected; insert the key that holds an age identity"
    return 0
  fi

  local runtime_dir="${XDG_RUNTIME_DIR:-/dev/shm}"
  [ -d "$runtime_dir" ] && [ -w "$runtime_dir" ] || runtime_dir="$(dirname "$identity")"
  SOPS_IDENTITY_TMPDIR="$(umask 077 && mktemp -d --tmpdir="$runtime_dir" ks-age-identity.XXXXXX)" || return 0
  trap sops_identity_cleanup EXIT INT TERM

  local filtered="$SOPS_IDENTITY_TMPDIR/identities.txt"
  if ! (umask 077 && awk -v serials="$present" '
    BEGIN {
      n = split(serials, list, "\n")
      for (i = 1; i <= n; i++) connected[list[i]] = 1
      keep = 0
      have_serial = 0
    }
    /^#[[:space:]]*[Ss]erial:/ {
      serial = $0
      sub(/^#[[:space:]]*[Ss]erial:[[:space:]]*/, "", serial)
      sub(/[^0-9].*$/, "", serial)
      keep = (serial in connected)
      have_serial = 1
      if (keep) print
      next
    }
    /^AGE-PLUGIN-YUBIKEY-/ {
      # A stub with no serial comment of its own cannot be attributed to a
      # key; without this it inherits the decision made for the stub above.
      # bin/sops-with-present-yubikey in ks-config makes the same check. Keep
      # the two in step until the filter lives in one place.
      if (!have_serial) exit 2
      if (keep) print
      keep = 0
      have_serial = 0
      next
    ...
    { print }
  ' "$identity" >"$filtered"); then
    note "each YubiKey identity in $identity must have its own Serial comment"
    return 0
  fi

  grep -q '^AGE-PLUGIN-YUBIKEY-' "$filtered" || {
    note "no connected YubiKey matches an identity in $identity"
    return 0
  }

  export SOPS_AGE_KEY_FILE="$filtered"

  # Say what is about to be asked for. The three PINs on one key are easy to
  # confuse, and each has its own retry counter and its own recovery path.
  local serial_list
  serial_list="$(printf '%s' "$present" | tr '\n' ' ')"
  note "decrypting with the PIV applet on YubiKey ${serial_list% }(age-plugin-yubikey)"
  note "PIV PIN, 6-8 characters — not the FIDO2 PIN used by ssh and LUKS; 3 wrong tries need the PUK"
}
...
cmd_secrets() {
  ...
  edit)
    ...
    sops_age_identity
    # Not exec: the EXIT trap must run to remove the filtered identity file.
    sops "$@"
    exit $?
    ;;
  sync) secrets_sync ;;
