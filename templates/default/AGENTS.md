...
- [`docs/keystone/github-token.md`](docs/keystone/github-token.md) — set up
  a sops-encrypted GitHub PAT so the host doesn't hit the 60/hr anonymous
...
- `hosts/<name>/configuration.nix` — host-only overrides. For Linux hosts
  ...
  system, no sops secrets, just user-scope packages and dotfiles. The file in
...
- `secrets/*.yaml` + `secrets/recipients.nix` — sops-encrypted secrets and
  the recipients that can decrypt them (`.sops.yaml` is generated by
  `ks secrets sync`). `secrets/` is empty by default; you only populate it
  from Step 8 of onboarding onward.
...
**Secrets never land in the Nix store.** sops-nix decrypts at activation
time into `/run/secrets/<name>` (root-owned by default; set `owner`/`mode` to
widen access). Read them at runtime — for example, a shell-init hook that
sources the file into the env — rather than via `home.sessionVariables` or
`nix.settings.access-tokens`, both of which embed the value at evaluation time
and end up world-readable in `/nix/store`.

**`sops-nix` is not a separate flake input.** Keystone's `operating-system`
module already imports `sops-nix.nixosModules.sops`, so
`keystone.secrets.provided.*` is available on every host with no extra
plumbing.
